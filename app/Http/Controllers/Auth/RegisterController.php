...
//    protected $redirectTo = RouteServiceProvider::STUDENT_PROFILE;
    ...
    protected function create(array $data)
    {
        ...
        $user_role = Role::find($data['role_id']);

        $pending = Role::where('name', 'pending')->first();

        $roles [] = $data['role_id'];

        if ($user_role->name == 'assistant' || $user_role->name == 'doctor')
            $roles[] = $pending->id;
